...
public class Ex01배열기초 {
	...
	public static void main(String[] args) {
		...
		//1. 같은 자료형만 저장할 수 있다.
		//2. 인덱스 번호를 가지고 있다.
		//   인덱스 번호는 0번부터 시작하여 차례로 1씩 증가
		//3. 크기가 고정적이다.
		
		//배열 선언하는 방법
		//자료형[] 변수명 = new 자료형[몇칸으로 만들지 숫자 입력];
		int[] numbers = new int[5];
		System.out.println(numbers);
		// numbers --->> 5칸짜리 배열이 어디 있는지
		//               주소값을 참조하는 레퍼런스 변수

		// new ---> 레버런스 변수 생성하는 키워드
		// 주소값이라는 개념이 왜 등장했나요?
		// :  자바에서는 데이터를 저장할수 있는 공간이 분리되어 있다.
		// --> 서로 다른 저장공간에 있는 데이터를 불러오기 위해서는
		// 해당하는 데이터가 어디에 저장되어 있는지 주소값이 필요하다.
		
		// 1) 스택(Stack)영역
		// : 비교적 작은 양의 데이터를 저장하는 공간
		// 2) 힙 (Heap)영역
		// : 비교적 커다랗고, 복잡한 데이터를 저장하는 공간
		// --> Garbage Collector(GC)라는 기능은
